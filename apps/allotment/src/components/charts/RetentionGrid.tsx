...
import { CohortRow } from '../../lib/metrics';
...
function cellStyle(value: number, mode: 'light' | 'dark'): React.CSSProperties {
  ...
  return {
    background: `color-mix(in oklab, var(--accent) ${Math.round(strength * 92)}%, var(--surface-solid))`,
    color: strength > INK_FLIP[mode] ? 'var(--accent-ink)' : 'var(--ink)',
  };
}
          ...
          {(['revenue', 'logos'] as const).map((option) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-[11px] font-bold capitalize transition-colors cursor-pointer ${
                mode === option ? 'bg-accent text-accent-ink' : 'bg-inset text-ink-soft hover:bg-inset-strong'
              }`}
            >
            ...
            {shown.map((row) => (
              <tr key={row.cohort}>
                ...
                {Array.from({ length: columns }, (_, offset) => {
                  ...
                  if (value === undefined) {
                    return <td key={offset} className="rounded-md bg-inset/40" />;
                  }
                  return (
                    <td
                      key={offset}
                      title={`${row.label}, month ${offset}: ${percent(value, 0)}`}
                      className="rounded-md py-1 text-center font-bold tabular-nums"
