...
import { type GardenState, type PlanTier, type Plant, type WeatherState } from '../types';
...
import { type CanvasTokens } from '../lib/theme';
import { usePageVisible, useReducedMotion } from '../lib/useReducedMotion';
import { cx } from './hud/ui';
import { type Particle, drawAmbient, seedAmbient, tileNoise } from '../lib/ambient';
import { type MetricCard, type MetricId } from '../lib/metricPlants';
import { type GardenPlanting, type ViewBed } from '../lib/gardenViews';
import { TILE_H, TILE_W, gridToUnscaled } from '../lib/iso';
import { type BedMarker, computeBedPlacement } from '../lib/plotLayout';
...
/**
 * **The plot is fixed in its frame.**
 *
 * Pan, wheel-zoom, the zoom dock and the camera flight are all off. The scene is
 * already laid out to *fit* — `computePlot` solves for the scale that puts every bed,
 * the metric border and its labels inside the viewport — so at rest the whole book is
 * on screen, and every camera move from there was a way to end up looking at part of
 * it and wondering where the rest went. A dashboard read at a glance should not have
 * a lost state.
 *
 * Nothing was deleted: the camera, the fit solver and the flight are intact behind
 * this one constant. Set it to `false` and panning, zooming, the dock and the
 * fly-to-customer all come back.
 */
const STATIC_VIEW: boolean = true;

/**
 * How far up the frame the fixed scene sits, in pixels.
 *
 * The fit solver centres the plot in the viewport, which is correct for a bare
 * canvas and slightly wrong for this one: the bottom of the screen carries the
 * timeline bar and the top carries only chrome the eye skips. Lifting the scene a
 * little puts the *plot* in the middle of what a reader actually looks at rather
 * than in the middle of the window.
 */
const STATIC_LIFT = 90;
...
export const IsometricGardenCanvas: React.FC<IsometricGardenCanvasProps> = ({
  ...
  const cameraRef = useRef<Camera>({ zoom: 1, x: 0, y: STATIC_VIEW ? -STATIC_LIFT : 0 });
  ...
  useEffect(() => {
    ...
    // Static: the whole plot is already in frame, so a request to "go to" a
    // subscription only has to *mark* it. Flying a fixed camera would be the one
    // thing that moved the scene.
    if (STATIC_VIEW) {
  ...
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    // Left un-prevented on purpose while static: swallowing the wheel on a fixed
    // scene would only stop the page behind it from scrolling.
    if (STATIC_VIEW) return;
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (event.buttons === 1 && !STATIC_VIEW) {
  ...
  const resetView = () => {
    cameraRef.current = { zoom: 1, x: 0, y: STATIC_VIEW ? -STATIC_LIFT : 0 };
    flightRef.current = null;
    setZoomDisplay(1);
    redrawIfStill();
  };
    ...
    const render = () => {
      ...
      if (border && metricCards && metricCards.length > 0) {
        ...
        border.specimens.forEach(({ index, gx, gy }) => {
          ...
          const sway =
            // Specimens sway only in the garden. `city` was once called
            // `cube`, and this read `shape === 'cube'` — a comparison that has
            // been quietly false, and the border swaying in a skyline, ever
            // since the rename.
            reducedMotion || shape !== 'tree' ? 0 : Math.sin(seconds * 1.4 + index * 1.7) * 1.2;
  ...
  return (
    ...
    >
      <canvas
        ...
        // A grab cursor over a scene that cannot be grabbed is a promise the plot
        // does not keep; `default` says "look and click", which is what is left.
        className={cx(
          'w-full h-full object-contain bg-transparent',
          STATIC_VIEW ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'
        )}
      />
      ...
      {!STATIC_VIEW && (
        <div className="absolute bottom-20 left-6 z-30 flex items-center gap-1 rounded-full border border-hairline bg-surface p-1.5 text-ink-soft shadow-panel backdrop-blur-md">
          <button
            ...
            className="rounded-full p-2 transition-colors hover:bg-garden-wash hover:text-garden-soft cursor-pointer"
            title="Zoom in (+)"
            aria-label="Zoom in"
          >
