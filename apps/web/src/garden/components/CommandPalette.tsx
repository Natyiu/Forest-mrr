  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-lg overflow-hidden rounded-[20px] border border-hairline bg-surface-solid shadow-modal"
      >
              ...
              return (
                <React.Fragment key={row.key}>
                  ...
                  <button
                    ...
                    onClick={() => run(row)}
                    className={cx(
                      'flex w-full items-center gap-2.5 rounded-2xl px-2.5 py-2 text-left transition-colors cursor-pointer',
                      isActive ? 'bg-garden-wash' : 'hover:bg-inset'
                    )}
                  >
                    {row.kind === 'plant' ? (
                      <>
                        <Sprout className="h-4 w-4 shrink-0 text-garden-soft" />
                      ...
                      <>
                        ...
                        {row.command.combo && (
                          <kbd className="shrink-0 rounded-lg border border-hairline bg-inset px-1.5 py-0.5 text-[10px] font-semibold text-ink-faint">
                            {comboLabel(row.command.combo)}
                          </kbd>
                        )}
                      </>
                    )}
                    {isActive && <CornerDownLeft className="h-3.5 w-3.5 shrink-0 text-garden-soft" />}
                  </button>
                </React.Fragment>
              );
            })
