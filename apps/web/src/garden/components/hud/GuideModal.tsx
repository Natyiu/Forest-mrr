...
import { type Hotkey, comboLabel } from '../../lib/useHotkeys';
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-[20px] border border-hairline bg-surface-solid p-6 shadow-modal"
        ...
        <ul className="mt-5 flex flex-col gap-4">
          {rules().map((rule, index) => (
            <li key={rule.title} className="flex gap-3">
              <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-xl bg-garden-wash text-[12px] font-bold text-garden-soft">
                {index + 1}
              </span>
              <span>
            ...
            <dl className="mt-2.5 flex flex-col gap-1.5">
              {hotkeys.map((hotkey) => (
                <div key={hotkey.combo} className="flex items-baseline justify-between gap-3">
                  ...
                  <dd>
                    <kbd className="rounded-lg border border-hairline bg-inset px-1.5 py-0.5 text-[11px] font-semibold text-ink-soft">
