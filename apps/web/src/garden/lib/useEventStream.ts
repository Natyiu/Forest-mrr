...
import { type PaymentEvent, type Plant } from '../types';

/**
 * The live wire.
 *
 * The server has always broadcast every payment, failure, upgrade and churn
 * over `/api/garden/stream`; nothing was listening. The garden updated only for the
...
export function useEventStream(handlers: StreamHandlers, enabled = true): StreamStatus {
  ...
  React.useEffect(() => {
    ...
    const source = new EventSource('/api/garden/stream');
